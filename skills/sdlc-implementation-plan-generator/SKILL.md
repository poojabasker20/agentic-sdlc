...
4. **PR Artifact Output**: Generate the finalized implementation plan using the **Standard Output Format Specification** and write it to `implementation-plans/<story-id>/<subtask_id>/plan.md` on `SDLC_GOVERNANCE_REPO`.

5. **Pull Request Creation**: Push branch `feature/plan-<subtask_id>` and open a GitHub PR titled `docs(<subtask_id>-plan): detailed implementation blueprint` targeting `main` for human tech lead review.
...
```markdown
# Implementation Blueprint: [Subtask Title]

**Subtask ID:** [e.g., SUBTASK-STORYID-1]  
...
- [ ] Implementation plan was generated from target subtask (`tasks/<story_id>/subtasks.md`) and parent story (`user-stories/<story_id>.md`).
